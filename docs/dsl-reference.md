...
| `moduleVersion` | string | no | Semantic version tag for module publishing (e.g. `v0.2.2`). Defaults to `v0.2.2`. |
...
| `startupHandler` | string | yes | URL path for the startup probe endpoint. Empty to disable. |
| `readinessHandler` | string | yes | URL path for the readiness probe endpoint. Empty to disable. |
| `livenessHandler` | string | yes | URL path for the liveness probe endpoint. Empty to disable. |
| `kubernetesWorkloadType` | enum | yes | Kubernetes controller for the service: `Deployment` or `StatefulSet`. |
