...
type grpcNoStreamingSinkConsumer[HandlerState, ReqT, ResR, T, R, E any] struct {
	...
	clientFn NoStreamingClientFunction[ReqT, ResR]
}
...
func (ec *grpcNoStreamingSinkConsumer[HandlerState, ReqT, ResR, T, R, E]) Consume(ctx context.Context, value T) {
	...
	// A unary RPC is a new request. Keep handlerCtx for returning the result
	// to the parent graph and give only the network request a fresh stream ID.
...
func (ec *grpcNoStreamingSinkConsumer[HandlerState, ReqT, ResR, T, R, E]) Start(_ context.Context) error {
	return nil
}

func (ec *grpcNoStreamingSinkConsumer[HandlerState, ReqT, ResR, T, R, E]) Stop(_ context.Context) {
}
...
func MakeGRPCNoStreamingEndpointConsumer[HandlerState, ReqT, ResR, T, R, E any](
	...
	if handler == nil {
		return nil, fmt.Errorf("handler is nil for GRPCNoStreamingSinkConsumer for the stream %q", stream.GetName())
	}
	...
	if _, ok := endpoint.GetConfig().(*config.GrpcEndpointConfig); !ok {
		return nil, fmt.Errorf("invalid endpoint config type for GRPCNoStreamingSinkConsumer for the stream %q", stream.GetName())
	}
	var tr tracing.Tracer
	if t := env.Tracing(); t != nil {
		tr = t.Tracer(env.ServiceConfig().Name)
	}
	ec := &grpcNoStreamingSinkConsumer[HandlerState, ReqT, ResR, T, R, E]{
...
func (ec *grpcNoStreamingSinkConsumer[HandlerState, ReqT, ResR, T, R, E]) GetID() int {
	return ec.Endpoint().GetID()
}

func (ec *grpcNoStreamingSinkConsumer[HandlerState, ReqT, ResR, T, R, E]) FunctionImplementation() interface{} {
	return ec.handler
}
