...
func (ds *saramaKafkaDataSink) Start(ctx context.Context) error {
	...
	for i := 0; i < endpoints.Len(); i++ {
		endpointCfg, ok := endpoints.At(i).GetConfig().(*config.KafkaEndpointConfig)
		if ok && endpointCfg.Enabled {
			enabled = true
			break
		}
	}
	...
	if err := saramatelemetry.Register(
		ds.GetEnvironment().Metrics(), kafkaConfig.MetricRegistry,
		saramatelemetry.RoleProducer, "",
	); err != nil {
		return err
	}
