...
lib.dashboard(
  ...
  panels=[
    ...
    lib.ts(
      title='GC Pause Duration p50 / p75 / max',
      targets=[
        lib.promQ(
          'go_gc_duration_seconds{%s, quantile="0.5"}' % jobFilter,
          'p50 {{job}}'
        ),
        lib.promQ(
          'go_gc_duration_seconds{%s, quantile="0.75"}' % jobFilter,
          'p75 {{job}}'
        ),
        lib.promQ(
          'go_gc_duration_seconds{%s, quantile="1"}' % jobFilter,
          'max {{job}}'
        ),
      ],
      w=12, h=8,
    ...
    lib.ts(
      title='GC Target and Runtime Parallelism',
      targets=[
        lib.promQ('go_gc_gogc_percent{%s}' % jobFilter, 'GOGC {{job}}'),
        lib.promQ('go_sched_gomaxprocs_threads{%s}' % jobFilter, 'GOMAXPROCS {{job}}'),
      ],
      w=12, h=8,
      unit='short',
    ),
