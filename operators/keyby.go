...
func (s *KeyByStream[T, K, V]) FunctionImplementation() interface{} {
	return s.f.f
}

func (s *KeyByStream[T, K, V]) GetErrorConsumer() runtime.RuntimeStream {
	return nil
}

func (s *KeyByStream[T, K, V]) GetValueType() reflect.Type {
	var kv datastruct.KeyValue[K, V]
	return reflect.TypeOf(&kv).Elem()
}

func (s *KeyByStream[T, K, V]) GetKeyType() reflect.Type {
	var k K
