...
func (s *SplitStream[T]) Consume(ctx context.Context, value T) {
    if s.TracingEnabled(ctx) {
        newCtx, span := s.StartSpan(ctx, "stream.split")
        ctx = newCtx
        defer span.End()
    }
...
func (s *SplitStream[T]) Build() error {
    ...
    sort.SliceStable(s.links, func(i, j int) bool {
        iAsync := s.links[i].caller != nil && s.links[i].caller.IsAsync()
        jAsync := s.links[j].caller != nil && s.links[j].caller.IsAsync()
        return iAsync && !jAsync
    })
    return nil
}
