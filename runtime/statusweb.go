...
func makeNodeImageURI(iconPath, bgColor string) string {
    svg := fmt.Sprintf(
        `<svg xmlns="http://www.w3.org/2000/svg" width="60" height="60"><rect width="60" height="60" rx="10" fill="%s"/><svg x="10" y="10" width="40" height="40" viewBox="0 0 24 24"><path d="%s" fill="white"/></svg></svg>`,
        bgColor, iconPath,
    )
    return svgDataURI(svg)
}

func makeNodeImageSelectedURI(iconPath, bgColor string) string {
    svg := fmt.Sprintf(
        `<svg xmlns="http://www.w3.org/2000/svg" width="60" height="60"><rect width="60" height="60" rx="10" fill="%s"/><svg x="10" y="10" width="40" height="40" viewBox="0 0 24 24"><path d="%s" fill="white"/></svg><rect x="2" y="2" width="56" height="56" rx="9" fill="none" stroke="#00FF80" stroke-width="4"/></svg>`,
        bgColor, iconPath,
    )
    return svgDataURI(svg)
}
...
func (app *ServiceApp) makeNode(runtimeStream RuntimeStream) *Node {
    ...
    n := &Node{
        ...
        Shape: "image",
        Image: nodeImage{
            Unselected: makeNodeImageURI(iconPath, background),
            Selected:   makeNodeImageSelectedURI(iconPath, background),
        },
        Size:    30,
