...
func (ec *netHTTPEndpointTypedConsumer[HandlerState, ReqT, ResR, T, R, E]) serveHTTP(w http.ResponseWriter, r *http.Request) {
	...
	case <-handlerCtx.Done():
		...
		ec.pending.Pop(streamID)
		ec.Endpoint().OnPendingRemove(handlerCtx, streamID)
		// A result callback holds result.mu for reading while it writes the
		// response and calls Done. If the request deadline and Done become ready
		// together, select may choose the cancellation branch first. Re-check
...
// completedResultWins resolves the only ambiguous state in the request wait:
// both the result and the request deadline may become ready before select runs.
// Once the result callback has released result.mu, a closed done channel means
// that the response is complete and must not be followed by an error response.
func completedResultWins(done <-chan struct{}) bool {
	select {
	case <-done:
		return true
	default:
